...
3. С помощью команды `!syncblk` увидел, что те же самые потоки (4 и 5) владеют активными блокировками, которые находятся в статусе MonitorHeld = 3 (то есть есть ожидающие блокировку другие потоки)
...
Free            0
```
4. Таким образом можно предположить, что оба потока 0x2f70 (4) и 0x6b90 (5) ожидают блокировки, захваченные друг-другом. То есть наблюдаем взаимную блокировку (deadlock). Блокировка выполнена через `lock`, так как он является синтаксическим сахаром для `System.Threading.Monitor.Enter`. Заблокированные объекты: 0000027a8b09f0c8 System.Object и 0000027a8b09f0e0 System.Object